-- Aditivní varianta RPC ukládá expiraci už při atomickém založení interní platby.
-- Původní overload zůstává dostupný pro bezpečný blue/green rollout starší verze aplikace.
...
begin
  ...
  if p_expires_at is null or p_expires_at <= clock_timestamp() then
    raise exception 'expires_at musí být v budoucnosti' using errcode = '22023';
  end if;
  ...
  if found then
    ...
    if v_existing_reservation.user_id <> p_user_id
      ...
      -- Expirace je neměnný snapshot operace. Tato podmínka odmítne i platbu
      -- vytvořenou starým overloadem, která má expires_at = null.
      or v_existing_payment.expires_at is distinct from p_expires_at
    then
...
revoke all on function public.create_payment_reservation(uuid, bigint, date, time, time, text, text, integer, timestamptz, text, jsonb) from public;
revoke all on function public.create_payment_reservation(uuid, bigint, date, time, time, text, text, integer, timestamptz, text, jsonb) from authenticated;
revoke all on function public.create_payment_reservation(uuid, bigint, date, time, time, text, text, integer, timestamptz, text, jsonb) from anon;
grant execute on function public.create_payment_reservation(uuid, bigint, date, time, time, text, text, integer, timestamptz, text, jsonb) to service_role;
