-- Expirace založená create_payment_reservation je neměnný snapshot celé operace.
-- Stavový přechod ji musí potvrdit, ale nesmí ji znovu vypočítat ani přepsat.
...
begin
  ...
  if p_new_status = 'awaiting_payment'
    and v_old_payment.expires_at is distinct from p_expires_at
  then
    raise exception 'expires_at se neshoduje s uloženým snapshotem platby' using errcode = '22023';
  end if;
...
comment on function public.record_payment_state_change(
  ...
  'Serverový RPC kontrakt pro změny platebního stavu. Execute má pouze service_role; funkce povoluje jen explicitní přechody, vyžaduje shodu expires_at s neměnným snapshotem, odvozuje auditní event_type a nemění částku, měnu, expiraci, idempotency_key ani reservation_id.';
